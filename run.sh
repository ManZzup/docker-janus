...
# CFG_EVENT='/root/janus/etc/janus/janus.eventhandler.sampleevh.jcfg'
# sed 's/enabled = no/enabled = yes/1' -i $CFG_EVENT
# echo 'backend = http://localhost:7777' >> $CFG_EVENT
# CFG_JANUS='/root/janus/etc/janus/janus.jcfg'
# sed 's/; broadcast = yes/broadcast = yes/1' -i $CFG_JANUS
# CFG_HTTPS='/root/janus/etc/janus/janus.transport.http.jcfg'
...
CMD="/root/janus/bin/janus --stun-server=stun.l.google.com:19302 -L /var/log/meetecho --rtp-port-range=10000-10200 --event-handlers"
until $CMD
do
    :
done
